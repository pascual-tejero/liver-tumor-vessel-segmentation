...
2D fully connected conditional random field post-processing optimization
PS: 3D CRF consumes a lot of memory, so if you don't have enough memory, you can consider using 2D.
"""
...
for file_index, file in enumerate(os.listdir(para.test_ct_path)):
    ...
    # Cut out the predicted results section to reduce crf processing difficulties
    ...
    for slice_index in tqdm(range(new_ct_array.shape[0])):
        ...
        # Acquisition of binary potential
...
# Writing evaluation indicators to the exel
